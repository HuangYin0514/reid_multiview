###########################################################
# Wandb
###########################################################
wandb login c74133df8c2cf575304acf8a99fe03ab74b6fe6a

###########################################################
# Runs
###########################################################
# python main.py --config_file "config/method.yml" TASK.NOTES=v859_visualization TASK.NAME=R_H2GP_S2GA_M2PQF_C2Mkl

# python main.py --config_file "config/method.yml" TASK.NOTES=v859_visualization TASK.NAME=R_H2GP_S2GA_M2PQF_C2Mkl MODEL.MODULE=Baseline
python main.py --config_file "config/method.yml" TASK.NAME=R_H2GP_S2GA_M2PQF_C2Mkl TASK.NOTES=v859_PART_NUM_5 MODEL.MODULE=Lucky MODEL.PART_NUM=2

###########################################################
# Visualize
###########################################################
mkdir -p results/outputs/models
wget -O results/outputs/models/model_114.pth "https://storage.googleapis.com/kaggle-script-versions/251889179/output/reid_multiview/main/results/outputs/models/model_114.pth?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=gcp-kaggle-com%40kaggle-161607.iam.gserviceaccount.com%2F20250728%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20250728T022543Z&X-Goog-Expires=3600&X-Goog-SignedHeaders=host&X-Goog-Signature="

# python main.py --config_file "config/method.yml"  TEST.RESUME_TEST_MODEL=114 
python main.py --config_file "config/method.yml" TASK.NAME=R_H2GP_S2GA_M2PQF_C2Mkl TASK.NOTES=v860_Baseline_visualization TASK.MODE=visualization MODEL.MODULE=Baseline

tar -czf results.tar.gz results
rm -rf results/outputs/actmap results/outputs/rank results/outputs/tSNE 
