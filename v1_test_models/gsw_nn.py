...
class GSW_NN:
    ...
    def gsw(self, X, Y, Z, random=True):
        ...
        print(Xslices.shape, Yslices.shape, Zslices.shape)
        ...
        print(Xslices_sorted.shape, Yslices_sorted.shape, Zslices_sorted.shape)
        print(torch.sort(Xslices, dim=0)[1])
...
if __name__ == "__main__":
    feat_l = torch.randn(64, 2048)
    feat_ab = torch.randn(64, 2048)
    feat_ori = torch.randn(64, 2048)
    gsw = GSW_NN("cpu", 1e-5, din=2048, num_filters=128, nofprojections=702, model_depth=3, train_wd=0.0005)
    output = gsw.max_gsw(feat_l, feat_ab, feat_ori)
    print(output)
