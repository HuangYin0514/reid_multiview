...
class TransLayer_1(nn.Module):
    def __init__(self):
        ...
        input_channel_list = [64, 256, 512, 1024]
        output_channel_list = [64, 64, 64, 64]
        kernel_size_list = [(4, 4), (4, 4), (2, 2), (1, 1)]
        cv1_list = nn.ModuleList()
        for i in range(self.num_layer):
            temp = nn.Sequential(
                nn.MaxPool2d(kernel_size=kernel_size_list[i]),
                nn.Conv2d(input_channel_list[i], output_channel_list[i], kernel_size=1, stride=1, padding=0, bias=True),
                nn.BatchNorm2d(output_channel_list[i]),
            )
            cv1_list.append(temp)
        self.cv1_list = cv1_list

        cv_fusion_list = nn.ModuleList()
        for i in range(self.num_layer):
            temp = nn.Sequential(
                nn.Conv2d(64, 64, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(64),
            )
            cv_fusion_list.append(temp)
        self.cv_fusion_list = cv_fusion_list
    ...
    def forward(self, xs):
        ...
        for i in range(self.num_layer):
            ...
            if i != 0:
                ...
                cv_feats = self.cv_fusion_list[i](cv_feats)
