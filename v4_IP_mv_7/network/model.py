...
class FeatureMapIntegrating(nn.Module):
    ...
    def __call__(self, bn_features, pids):
        ...
        shared_features, special_features = bn_features.chunk(2, dim=1)

        # Fusion
        integrating_bn_features = special_features.view(chunk_bs, 4, -1)  # (chunk_size, 4, f_dim)
        integrating_bn_features = torch.mean(integrating_bn_features, dim=1).repeat_interleave(4, dim=0)

        integrating_bn_features = torch.cat([shared_features, integrating_bn_features], dim=1)
